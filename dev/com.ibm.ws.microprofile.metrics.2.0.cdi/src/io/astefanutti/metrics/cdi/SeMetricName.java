...
/* package-private */ class SeMetricName implements MetricName {
    ...
    public Metadata metadataOf(Annotated annotated, String name, Class<?> type) {
        MetadataBuilder builder = Metadata.builder().withName(name).withType(MetricType.from(type));
        if (annotated.isAnnotationPresent(Metric.class)) {
            Metric metric = annotated.getAnnotation(Metric.class);
            String desc = metadataValueOf(metric.description());
            if (desc != null && !desc.trim().isEmpty()) {
                builder.withDescription(desc);
            }
            String displayName = metadataValueOf(metric.displayName());
            if (displayName != null && !displayName.trim().isEmpty()) {
                builder.withDescription(displayName);
            }
            builder.withUnit(metadataValueOf(metric.unit()));
